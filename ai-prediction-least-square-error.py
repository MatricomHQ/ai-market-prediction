...
#print(f"test index length: {len(csv_file.index.isin(idx_test))}")
...
#actual_vs_predicted = pd.DataFrame({"Actual": csv_file.loc[csv_file.index.isin(X_test.reshape(-1)), "spy_price"], "Predicted": predictions})
print(tabulate(actual_vs_predicted, headers='keys', tablefmt='fancy_grid'))
'''
...
next_direction = ["up" if p > a else "down" for a, p in zip(actual_vs_predicted["Actual"], actual_vs_predicted["Predicted"])]
