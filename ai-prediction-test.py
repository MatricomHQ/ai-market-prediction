...
# Split data using the half index
X_train, X_test = X[:half_idx], X[half_idx:]
y_train, y_test = y[:half_idx], y[half_idx:]

# Split data for test indices
test_indices = csv_file.index[half_idx:]
...
# Create and compile GRU model with Adagrad optimizer
model = Sequential()
...
model.compile(optimizer=tf.keras.optimizers.Adagrad(learning_rate=0.003), loss='binary_crossentropy')
